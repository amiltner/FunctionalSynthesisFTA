...
module Compute(C : FTASynthesizer.AutomatonBuilder) = struct
  module A =
  struct
    module Symb = struct
      type t = (Id.t * int)
  ...
  let get_states
            ...
            List.fold
              ~f:(fun a varg ->
                  A.add_transition
                    a
                    (i, List.length vs)
                    (List.map ~f:(fun (v,t) -> (A.St.Vals ([(varg,v)],t))) vs)
                    (A.St.Vals ([(varg,v)],t)))
              ~init:a
              relevant_vs)
        ~init:a
    ...
    let a =
        ...
        ~f:(fun acc (s1,s2) ->
            begin match (s1,s2) with
             | (A.St.Vals ([v11,v12],_), Vals ([v21,v22],t)) ->
               if List.mem
                   ~equal:(fun ex1 ex2 ->
                       is_equal
                         (pair_compare
        ...
        ~init:a
        (List.cartesian_product (A.states a) (A.states a))
    in
    let a =
      List.fold
        ~f:(fun a s ->
             ...
             | Vals ([(i,_)],_) ->
               let a =
                 A.add_transition
                   a
                   (Id.create "ifthenelse", 3)
                   [(A.St.Vals ([(i,Value.true_val)],fst (TypeDS.find_representative ds Type._bool)))
                   ;s
                   ;top]
                   s
               in
               let a =
                 A.add_transition
                   a
                   (Id.create "ifthenelse", 3)
                   [(A.St.Vals ([(i,Value.false_val)],fst (TypeDS.find_representative ds Type._bool)))
                   ;top
                   ;s]
                   s
               in
      ...
      fold_on_head_exn
        ~f:(fun x y ->
            ...
            *)
            ...
            print_endline (string_of_int ((List.length (A.states x))));
            print_endline (string_of_int ((List.length (A.states y))));
            print_endline "start my inter"; let x = A.minimize (A.intersect x y) in
    ...
    (List.iter ~f:(fun _ -> print_endline "beep") (A.final_states a););
    print_endline "END FINALS";



    let e = A.term_to_exp (A.pick_term a (*get_all_recursions' a*)) in
