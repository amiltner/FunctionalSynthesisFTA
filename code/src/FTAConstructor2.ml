...
module Compute(C : FTASynthesizer.AutomatonBuilder) = struct
  ...
  let perform_process
    ...
    in
    let c = update_from_conversions c conversions in
    let c = update_from_conversions c conversions in
    let c = add_destructors c in
    let c = add_let_ins c in
    let cs =
      List.map
        ~f:(fun (ins,out) ->
            let inp = ins in
    ...
    let a =
      fold_on_head_exn
        ...
        auts
    in
    let e = A.term_to_exp (A.pick_term a) in
