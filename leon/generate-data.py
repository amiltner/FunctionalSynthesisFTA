...
def gather_datum(path, base, additional_flags, timeout):
    ...
    print(["./leon","--synthesis"] + BASE_FLAGS + flags + [join(path, base + TEST_EXT)])
    process_output = EasyProcess(["./leon","--synthesis"] + BASE_FLAGS + flags + [join(path, base + TEST_EXT)]).call(timeout=timeout+5)
    ...
    def gather_col(flags, run_combiner, col_names, timeout_time, repetition_count, compare):
        ...
        for iteration in range(repetition_count):
            ...
            if datum == "":
                memout = True
                break
            startind = datum.find("Finished in ")
            initial = datum[startind+12:]
...
def main(args):
    if len(args) == 3:
        example_based_path = args[1]
        postconditional_path = args[2]
        exrootlength = len(example_based_path)
